...
public class UIUtils {
	...
	public static class Bean {
		...
		public static String getSentForm() {
			String form = "\r\n(Sent From IQQ For @osname.)";
			String osName = System.getProperty("os.name");
			osName = StringUtils.isEmpty(osName) ? "Unknown" : osName;
			form = form.replaceAll("@osname", osName);
			return form;
		}
	...
	public static class PinyinConvert{
		...
		public static String[] getPinyin(String str){
	    	...
	    	for (int i = 0; i < cr.length; i++) {
	    		if(isChinese(cr[i])){
		    		String[] pinyinArray =PinyinHelper.toHanyuPinyinStringArray(cr[i], spellFormat);
		    		for(int j = 0; j < pinyinArray.length; j++){
		    			temp_quanpin += pinyinArray[j].toString().replaceAll("\\d+", "");
		    			temp_jianpin += pinyinArray[j].charAt(0);
		    			break;
		        	}
	    		}else{
	    			crStr +=cr[i];
	    	...
	    	result[0] = crStr;	//名字中的符号
	    	result[1] = temp_jianpin;	//简拼
	    	result[2] = temp_quanpin;	//全拼
